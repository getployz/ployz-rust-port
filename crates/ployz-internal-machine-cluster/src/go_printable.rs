// The Go oracle formats duplicate-name errors with fmt %q. Its definition of
// printable Unicode is generated for Go's pinned Unicode version and intentionally
// differs from Rust's char::escape_debug. These tables are copied from the frozen
// Go 1.26.1 toolchain's strconv/isprint.go. See ../THIRD_PARTY_NOTICES.md for
// the retained Go Authors copyright and BSD-3-Clause terms.
...
pub(crate) fn is_print(character: char) -> bool {
    let rune = u32::from(character);
    if rune <= 0xff {
        return (0x20..=0x7e).contains(&rune) || ((0xa1..=0xff).contains(&rune) && rune != 0xad);
    }

    if rune < 0x10000 {
        return is_in_ranges(PRINT_16, rune) && NOT_PRINT_16.binary_search(&rune).is_err();
    }

    is_in_ranges(PRINT_32, rune)
        && (rune >= 0x20000 || NOT_PRINT_32.binary_search(&(rune - 0x10000)).is_err())
}

fn is_in_ranges(ranges: &[u32], rune: u32) -> bool {
    ranges
        .chunks_exact(2)
        .any(|range| range[0] <= rune && rune <= range[1])
}
