...
- **Controller:** read `migration/CONTROLLER.md`. Own migration registries,
  dependency-ready scheduling, and dynamic worker assignments. Do not implement
  crates.
...
- **Package implementor:** read the dynamic assignment built from
  `migration/PACKAGES.tsv`, dependency decisions, the oracle, tests, and
  callers. Own only the assigned crate directory. Use only approved
  dependencies. A missing dependency returns the package to the dependency
  gate.
